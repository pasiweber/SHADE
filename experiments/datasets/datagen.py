...
class densityDataGen:
    ...
    def __init__(
        ...
        if self.con_dens_factors == False:
            ...
            raise BaseException(
                "Shape of connection scale factors does not match connection number"
            )
        ...
        if verbose:
            for i in range(len(connection_starts)):
                print(
                    "Connection from "
                    + str(connection_starts[i])
                    + " to "
                    + str(connection_stops[i])
                    + " with factor "
                    + str(self.con_dens_factors[i])
                    + " and momentum "
                    + str(self.con_momentum[i])
                )
            ...
            while True:
                ...
                if noretry:
                    self.connections[conid] = (
                        str(connection_starts[i]) + "-" + str(connection_stops[i])
                    )
                    ...
                    if len(self.cores[conid]) == 0:
                        if retry >= self.max_retry:
                            ...
                            if self.verbose:
                                print(
                                    "Could not generate connection from "
                                    + str(connection_starts[i])
                                    + " to "
                                    + str(connection_stops[i])
                                    + " as they were too close"
                                )
                            break
                        else:
                            retry += 1
    ...
    def generate_cluster(self, cluid):
        ...
        while True:
            ...
            if noretry:
                ...
                retry >= self.max_retry and no_space
            ):  # expand domain size, at some point there is enough space for new cluster
    ...
    def make_connection(self, startid, stopid, conid):
        ...
        min_dist = (
            self.con_dens_factors[conind] * self.c_sphere
            + self.dens_factors[stopid] * self.r_sphere
        ) * self.con_min_dist
        retry_some = False
        tries_some = -1
        while dist > min_dist:
            ...
            while retry_last or retry_some:
                ...
                min_rand = 2 / 3
                step_randomness = max(
                    min_rand, min(max_rand, np.random.normal(1, self.step_spread, 1))
                )
                ...
                if self.tooclose(pos, conid, exclude=[startid, stopid, conid]):
                    ...
                    for d in range(self.dim):
                        self.mins[d] = min(
                            pos[d] - self.c_sphere * self.con_dens_factors[conind], self.mins[d]
                        )
                        self.maxs[d] = max(
                            pos[d] + self.c_sphere * self.con_dens_factors[conind], self.maxs[d]
                        )
        ...
        if len(connectors) > 0:
            ...
            pos = (
                ...
                + (
                    self.con_dens_factors[conind] * self.c_sphere
                    + self.dens_factors[startid] * self.r_sphere
                )
                * step_dir
                * self.con_min_dist
            )
            ...
            while retry_last or retry_some:
                ...
                if retry_some or rand < branch_chance:
                    ...
                    if star_chance > 0 and len(self.cores[cluid]) > 1:
                        probs = [star_chance]
                        probs.extend(
                            [(1 - star_chance) / (len(self.cores[cluid]) - 1)]
                            * (len(self.cores[cluid]) - 1)
                        )
                ...
                if self.tooclose(pos, cluid):
                    ...
                    for d in range(self.dim):
                        self.mins[d] = min(
                            pos[d] - self.r_sphere * self.dens_factors[cluid], self.mins[d]
                        )
                        self.maxs[d] = max(
                            pos[d] + self.r_sphere * self.dens_factors[cluid], self.maxs[d]
                        )
        ...
        for cluid in self.cores.keys():
            ...
            if equal:
                ...
                assignment = np.random.choice(
                    clu_core_num, clu_data_num % clu_core_num, replace=False
                )
    ...
    def paint(self, dim1, dim2, data=None, show_radius=True, show_core=True, cores=None):
        ...
        if data is not None:
            plt.scatter(
                data[:, dim1],
                data[:, dim2],
                color=color[data[:, len(data[0]) - 1].astype("int32") + 1],
                alpha=1,
            )
            ...
            if len(cores[cluid]) > 0:
                if self.verbose:
                    if cluid >= 0:
                        ...
                        patch = mpatches.Patch(
                            color=color[cluid + 1], label=self.connections[cluid]
                        )
                        legend.append(patch)
                ...
                if show_core:
                    if data is None:
                        ...
                        plt.scatter(
                            cores[cluid][:, dim1], cores[cluid][:, dim2], color="black", alpha=1
                        )
                if show_radius:
                    for core in cores[cluid]:
                        ...
                        if cluid >= 0:
                            ...
                            plt.gca().add_patch(
                                mpatches.Circle(
                                    (core[dim1], core[dim2]),
                                    radius=self.c_sphere * self.con_dens_factors[(-1 * cluid) - 2],
                                    color=color[cluid + 1],
                                    alpha=0.2,
                                )
                            )
            ...
            if command[i] == "(":
                ...
                if command[i + 1] == "c":
                    ...
                    for conkey in self.connections.keys():
                        if self.connections[conkey] == str(cid) + "-" + str(
                            eid
                        ) or self.connections[conkey] == str(eid) + "-" + str(cid):
                            cid = conkey
                        ...
                        if command[i + j] != ",":
                            ...
                            if command[i + j + 1] == ":":
                                ...
                                while command[i + j + 1].isnumeric():
                                    ...
                                    j += 1
                                cstopid = int(cstopid)
                                data_cores_block[block][cid].extend(
                                    np.arange(cstartid, cstopid + 1).tolist()
                                )
                            else:
    ...
    def __next__(self):
        ...
        if recalc_db:
            ...
            for cluid in self.stream_data_cores_block[block].keys():
                if cluid >= 1:
                    ...
                    block_db[cluid] = (
                        self.ratio_con
                        * len(self.stream_data_cores_block[block][cluid])
                        / con_core_num
                    )
                db_sum += block_db[cluid]

            for cluid in block_db.keys():
                block_db[cluid] = (block_db[cluid] / db_sum) * (
                    1 - self.noise_block[block] * self.ratio_noise
                )
        ...
        cluid_choice = np.random.choice(
            list(self.cur_stream_db.keys()), 1, p=list(self.cur_stream_db.values())
        )[0]
