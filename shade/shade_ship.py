...
class SHiP_Clusterer(ClusterMixin, BaseEstimator):
    def __init__(
            self,
            min_points: int = 5,
            k: int = 2,
    ...
    def fit(self, X, y=None):
        self.ship = SHiP(
            ...
            treeType="DCTree", 
            hierarchy=1, 
            partitioningMethod="K", 
            config={"relaxed": False, "k": self.k}
        )

        self.labels_ = self.ship.fit_predict()
        self.n_clusters = len(np.where(np.unique(self.labels_) >= 0)[0])

        return self


class SHADE_SHiP(BaseEstimator, ClusterMixin):
    """
    ...
    def __init__(
        ...
        cluster_algorithm: Optional[ClusterMixin] = SHiP_Clusterer,
        cluster_algorithm_params: dict = {},
        ...
        self.min_points = min_points
        self.use_complete_ship = use_complete_ship
        self.ship = None
        ...
        self.cluster_algorithm_params.update({"k": n_clusters})
    ...
    def fit(self, X, y=None) -> SHADE:
        ...
        self.ship = SHiP(data=X, treeType="DCTree")
...
class _SHADE_Module(_AbstractAutoencoder):
    ...
    def fit(
        ...
        self.matrix_dc_distance = self.ship.get_tree(0).distance_matrix()
